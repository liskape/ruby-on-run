module RubyOnRun::VM::InvokeInstructionInterpretation
  ...
  def send_stack_with_block(args, parameters = [], context)
    ...
    evaluate_block(result, parameters, context.current_class, context.self, block)
    context.push result
  end
  ...
  def evaluate_block(enumerator, parameters, current_class, current_object, block)
    enumerator.each do |x|      
      new_context = RubyOnRun::VM::Context.new(block.compiled_code, current_class, current_object, block.parent_context, {})
